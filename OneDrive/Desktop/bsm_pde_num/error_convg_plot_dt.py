...
dt_vals = [0.04, 0.02, 0.01, 0.005, 0.0025] # Values of dt for plot (in form dt,dt/2,dt/4,...)
...
## Plot 1 (time convergence on methods 1. implicit 2. cn with no rannacher 3. cn with rannacher)
plt.figure(figsize=(10, 5))

plt.loglog(dt_vals, max_err_imp_arr, 'o-', label='Implicit')
plt.loglog(dt_vals, max_err_cn_arr, 's-', label='CN')
plt.loglog(dt_vals, max_err_cn_ran_arr, '^-', label='CN + Rannacher')

plt.xlabel("Time step (dt)")
plt.ylabel("Max error")
slope_imp = round(np.polyfit(np.log(dt_vals), np.log(max_err_imp_arr), 1)[0])
plt.title(f"Time Convergence Study (log-log, ds=0.625, 1 year maturity, implict slope ≈ {slope_imp})")

# --- Force x-axis ticks to be exactly your dt values ---
plt.xticks(dt_vals, [f"{dt:g}" for dt in dt_vals])
...
## Plot 2 (Smoothening effect on CN without rannacher)
plt.figure(figsize=(10, 5))

plt.loglog(dt_vals, max_err_cn_arr, 'o-', label='No smoothening')
plt.loglog(dt_vals, max_err_cn_cell_arr, 's-', label='Cell average smoothening')
plt.loglog(dt_vals, max_err_cn_quad_arr, '^-', label='Quadratic smoothening')

plt.xlabel("Time step (dt)")
plt.ylabel("Max error")
plt.title(f"Effect of payoff smoothing on Crank Nicolson (no Rannacher, log-log, ds=0.625, 1 year maturity)")
